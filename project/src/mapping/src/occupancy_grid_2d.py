...
class OccupancyGrid2d(object):
    ...
    def LoadParameters(self):
        ...
        self._occ_topic = rospy.get_param("~topics/occ")
    ...
    def RegisterCallbacks(self):
        ...
        self._occ_pub = rospy.Publisher(self._occ_topic,
                                        OccupancyGrid,
                                        queue_size=10)
    ...
    def SensorCallback(self, msg):
        ...
        for idx, r in enumerate(msg.ranges):
            ...
            # TODO!
            angle = msg.angle_min + msg.angle_increment * idx + yaw
            ...
            # TODO!
            end_point_x = sensor_x + r*np.cos(angle)
            end_point_y = sensor_y + r*np.sin(angle)
            lastx, lasty = self.PointToVoxel(end_point_x,end_point_y)
            self._map[lastx][lasty] += self._occupied_update
            if self._map[lastx][lasty] > self._occupied_threshold:
                self._map[lastx][lasty] = self._occupied_threshold
            # x_range = np.arange(sensor_x, end_point_x, self._x_res)
            # y_range = np.arange(sensor_y, end_point_y, self._y_res)
            r_range = np.arange(0, r, self._x_res)
            last_voxel = [0,0]
            for arr in r_range:
                x = sensor_x + arr*np.cos(angle)
                y = sensor_y + arr*np.sin(angle)
                gx, gy = self.PointToVoxel(x,y)
                if (last_voxel[0] == gx and last_voxel[1] == gy) or (lastx == gx and lasty == gy):
                    pass
                else:
                    self._map[gx][gy] += self._free_update
                    if self._map[gx][gy] < self._free_threshold:
                        self._map[gx][gy] = self._free_threshold
                last_voxel[0] = gx
                last_voxel[1] = gy
